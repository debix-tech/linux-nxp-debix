...
#define ov_info(fmt, args...)   pr_info("ov5640: "fmt, ##args)
...
static int ov5640_set_stream_mipi(struct ov5640_dev *sensor, bool on)
{
	...
	line_sync = 1;/*(mode->id == OV5640_MODE_XGA_1024_768 ||
				  ...
				  struct v4l2_subdev_frame_size_enum *fse)
{
	...
	GLS("%s %d %d, index=%d\n", __func__, fse->min_width, fse->min_height, fse->index);
	...
	for (i = 0; i < OV5640_NUM_FRAMERATES; i++) {
		for (j = 0; j < OV5640_NUM_MODES; j++) {
			...
			if (fie->index == (count - 1)) {
				GLS("%s fir(wxh)=%d x %d ok!!!\n", __func__, fie->width,fie->height);
				fie->interval.denominator = ov5640_framerates[i];
				return 0;
			}
		}
	}
	GLS("%s fir(wxh)=%d x %d err !!!\n", __func__, fie->width,fie->height);
