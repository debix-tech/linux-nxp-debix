...
static ssize_t gpio_read (struct file *filp, char __user *buff, size_t count, loff_t *offp)
{
	...
	for(i=0; i<0x50; i++){
		value = snd_soc_component_read(es8316_component, i);
...
static ssize_t  gpio_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    sscanf(wrdata, "%x,%x", &reg,&value);
	snd_soc_component_write(es8316_component, reg, value);
	value = 0;
	value = snd_soc_component_read(es8316_component, reg);
	printk("%s: set 0x%02x = 0x%02x\n", __func__,reg,value);
	
	return count;
}

static int major;
static char class_name[30];
static struct cdev gpio_cdev;
static struct class *gpio_drv_class;
static struct device *gpio_drv_class_dev;
static struct file_operations gpio_fops;


static int gpio_register_chrdev(const char *name)
{
	dev_t devno = MKDEV(major,0);
	sprintf(class_name, "%s_class", name);
	
	major = 0;
	gpio_fops.owner = THIS_MODULE;
	gpio_fops.open = gpio_open;
	gpio_fops.write = gpio_write;
	gpio_fops.read = gpio_read;
	gpio_fops.release = gpio_release;

	if (alloc_chrdev_region(&devno, 0, 1, class_name) < 0)
	{
		printk(KERN_INFO "..alloc..reg err.\r\n");				
		return -1;
	}
	major = MAJOR(devno); //get the major after alloc register
	gpio_cdev.owner = THIS_MODULE;	
	cdev_init(&gpio_cdev, &gpio_fops);	
	if(cdev_add(&gpio_cdev,MKDEV(major,0),1) < 0)
	{
		printk("..add err.\r\n");
		return -1;
	}	

	gpio_drv_class = class_create(THIS_MODULE, class_name);
	if(IS_ERR(gpio_drv_class)){
		printk("failes 2 wg_drv register\n");
		goto gpio_class_create_fail;
	}

	gpio_drv_class_dev = device_create(gpio_drv_class, NULL, devno, 0, name);
	if(IS_ERR(gpio_drv_class_dev)){
		printk("failes  wg_drv register\n");
		goto gpio_dev_create_fail;
	}
	printk("%s: name=%s, class_name=%s, pdata->major=%d\n",__func__, name,class_name, major);
	return 0;
...
static const struct snd_kcontrol_new es8316_snd_controls[] = {
	...
	SOC_DOUBLE_TLV("HP Playback Volume", ES8316_CPHP_ICAL_VOL_REG18,
		       4, 0, 3, 1, hpout_vol_tlv), //  4, 0, 4, 1, hpout_vol_tlv)
	/* HPMIXER VOLUME Control */
	SOC_DOUBLE_TLV("HPMixer Gain", ES8316_HPMIX_VOL_REG16,
		       0, 4, 11, 0, hpmixer_gain_tlv), // 0, 4, 7, 0, hpmixer_gain_tlv)
...
//	printk("es8316 %s \n",__func__);
	//John_gao  set Lin2
	snd_soc_component_write(component, ES8316_ADC_PDN_LINSEL_REG22, 0x10);
	return 0;
}
...
static int es8316_mute(struct snd_soc_dai *dai, int mute, int direction)
{
	...
	#endif
	snd_soc_component_update_bits(dai->component, ES8316_DAC_SET1_REG30, 0x20,
			    mute ? 0x20 : 0);
	return 0;
}
...
static ssize_t es8316_reg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	printk("================ \n\n\n");
	for (i = 0 ; i < 0x53; i++){
		reg = snd_soc_component_read(es8316_component, i);
		printk("%02x: %02x \n", i , reg);
	}
...
static DEVICE_ATTR_RO(es8316_reg);
