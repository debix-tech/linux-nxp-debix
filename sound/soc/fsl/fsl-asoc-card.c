...
//end add by polyhex
	...
	if (priv->spk_vdd5v_en < 0) {
		...
		ret = devm_gpio_request_one(&pdev->dev, priv->spk_vdd5v_en,
					    GPIOF_OUT_INIT_LOW, "spk_vdd5v_en");
		if (ret) {
...
static void fsl_asoc_card_remove(struct platform_device *pdev)
{
